...
function EditarContato({ closeModal, contato }) {
    ...
    const [quantInputNovosTel, setQuantInputNovosTel] = useState(0);
    const [novosTelefones, setNovosTelefones] = useState(Array(quantInputNovosTel).fill(""));
    ...
    const observaInputNovosTelefones = (index, value) => {
        const novosValores = [...novosTelefones];
        novosValores[index] = value;
        setNovosTelefones(novosValores);
    };
    const observaInputTelefonesAntigos = (element, index, newNumber) => {
    ...
    function pegarQuantidadeNovosTelefones(e) {
        if (e.target.value == 0) {
            setQuantInputNovosTel(0)
            return
        }
        setQuantInputNovosTel(e.target.value)
    }
    ...
    async function atualizarContato() {
        const novosTelefonesEnviados = novosTelefones.slice(0, quantInputNovosTel)
    ...
    for (let i = 0; i < quantInputNovosTel; i++) {
        inputsTelefone.push(
            <div key={quantidades[i]} className="mb-4 relative">
                ...
                <input
                    ...
                    onChange={(e) => observaInputNovosTelefones(i, e.target.value)}
                ...
                <div className='w-[80%]'>
                        ...
                        return (
                            <div key={element.id} className="mb-4 relative">
                                ...
                                <input
                                    ...
                                    className="w-full border rounded-md py-2 px-3 text-blue-600 focus:outline-none focus:border-blue-500"
                                    placeholder={`Exemplo: 31985965572`}
                                    onChange={(e) => observaInputTelefonesAntigos(element, i, e.target.value)}
                    ...
                    <div className="mb-4">
                        ...
                        <select
                            ...
                            required
                            onChange={pegarQuantidadeNovosTelefones}
                        >
